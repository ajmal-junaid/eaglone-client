...
const CartBody = () => {
  ...
  const getData = () => {
    instance
      .get(`get-cart?userId=${params.id}`)
      .then((res) => {
        setCart(res.data.data);
      })
      .catch((res) => {
        console.log(res.response.data.message, "catch");
  ...
  const handleDelete = (courseId, title) => {
    ...
    setIsOpen(true);
    setDelete(title);
    setConfirmDialog({ active: true, id: courseId, name: title });
  };
  const handleDeletee = () => {
    instance
  ...
  function closeModal() {
    ...
    setConfirmDialog({ active: false, id: courseId });
  }
  return (
